...
KEYS=(
      ...
      'AIzaSyDbfKw4-tMvrLOp-YoT6_McH-g5JzvR6qw',
...
def printHelp():
    print("Usage: getLatLon_forA_Lvr.py -f FILE_NAME")
...
def getLatLon(filePath):
    ...
    for line in inFile:
        key=KEYS[index]
        if line.find("房地(土地+建物)") >1:
            if line.find("號") >= 0 and line.find("~")>=0:
                addr=line.split(',')[2].split("~")[0]+'號'
